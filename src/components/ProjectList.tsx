      ...
      <>
         <div className="mx-auto mt-14 grid grid-cols-1 gap-8 md:mx-0 md:grid-cols-2 xl:grid-cols-3">
            {projects.slice(0, displayProjects).map((project: projectType, index: number) => (
               <div key={index} className="bg-secondary group flex max-w-md flex-col justify-between overflow-hidden rounded-lg shadow-sm backdrop-blur-md transition-transform duration-300 hover:scale-102">
                  <div>
                     ...
                     <div className="p-5">
                        <h5 className="text-xl font-bold tracking-tight capitalize">{snakeToNormalCase(project.title)}</h5>
                        <div className="my-3">
                  ...
                  <div className="flex space-x-3 px-5 pt-2 pb-4">
                     <a
                        target="_blank"
                        href={project.github}
                        className="bg-third inline-flex items-center rounded-lg px-3 py-2 text-center text-xs font-medium text-white hover:bg-gray-900 focus:ring-4 focus:ring-blue-300 focus:outline-hidden md:text-sm">
         ...
         {displayProjects < projects.length && (
            <div className="mt-5 flex items-center justify-center">
               <button onClick={handleLoadMore} className="bg-fourth inline-flex items-center rounded-lg px-3 py-2 text-center text-xs font-medium text-white focus:ring-0 focus:outline-hidden md:text-sm">
                  {trlnsMore}
               </button>
            </div>
         )}
