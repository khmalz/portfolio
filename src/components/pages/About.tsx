...
import type { svgTechType } from "@/types/svgTechType";
import SvgTech from "@/components/SvgTech";

export default function AboutSection() {
   const trlns = useTranslations("about");
   const lang = useLocale();

   const techs: svgTechType[] = techsCollection;

   const isId = lang === "id";
   const cv = isId ? process.env.CV_ID : process.env.CV_EN;
   if (!cv) {
      console.warn(`CV_ID atau CV_EN belum di-set di .env`);
   }

   const cvUrl = `https://drive.google.com/file/d/${cv}/view`;

   return (
      <section id="about" className="container pt-16">
         <div className="bg-secondary flex flex-col justify-between space-y-8 rounded-xl px-5 py-10 text-white shadow-xl md:flex-row md:space-y-0 md:space-x-8">
            <div className="flex w-full flex-col space-y-5 backdrop-blur-lg backdrop-filter md:w-1/2">
               ...
               <a
                  ...
                  href={cvUrl}
                  download
                  className="hover:text-fourth focus:text-fourth inline-flex items-center self-baseline rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-900 hover:bg-gray-100 focus:z-10 focus:ring-4 focus:ring-gray-200 focus:outline-hidden">
